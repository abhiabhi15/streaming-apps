...
json_gz_filename = 'dblp_article.json.gz'
tmp_filename = 'tmp_dblp.json.gz'
report_frequency = 20000

class DBLPHandler (xml.sax.ContentHandler):
  papertypes = set (['article'])
  ...
  def startElement (self, name, attrs):
    ...
    elif name in ['author', 'year', 'title','journal']:
      self.text = ''
  ...
  def write_paper (self):
    ...
    json.dump ([self.paper,self.papertype, self.authors, self.year, self.title, self.journal], self.out)
